...
// Defaults — overridden by brandConfig at runtime
...
function getImageDimensions(buffer) {
  // Read dimensions from PNG or JPEG header
  ...
  return { width: 300, height: 90 }; // fallback
}

function createBrandedHeader(recruiter, logoPath, logoSizePx) {
  const logoCellChildren = [];
  if (logoPath && fs.existsSync(logoPath)) {
    const logoBuffer = fs.readFileSync(logoPath);
    const dims = getImageDimensions(logoBuffer);

    // Use custom size from config, scale height proportionally
...
async function generateDocx(cvData, recruiter, logoPath, outputPath, brandConfig) {
  applyBrand(brandConfig);
  const logoOpacity = brandConfig?.logoOpacity ?? 100;

  // If opacity < 100, pre-process logo to bake in transparency as a semi-transparent PNG
  let processedLogoPath = logoPath;
  if (logoOpacity < 100 && logoPath && fs.existsSync(logoPath)) {
    try {
      const puppeteer = require('puppeteer');
      const logoBase64 = fs.readFileSync(logoPath).toString('base64');
      const dims = getImageDimensions(fs.readFileSync(logoPath));
      const browser = await puppeteer.launch({ headless: 'new', args: ['--no-sandbox'] });
      const page = await browser.newPage();
      await page.setViewport({ width: dims.width + 20, height: dims.height + 20 });
      await page.setContent(`<html><body style="margin:0;background:transparent;">
        <img src="data:image/png;base64,${logoBase64}" style="opacity:${logoOpacity / 100};" />
      </body></html>`);
      const imgEl = await page.$('img');
      const opacityPath = logoPath.replace(/\.[^.]+$/, '_opacity.png');
      if (imgEl) await imgEl.screenshot({ path: opacityPath, omitBackground: true });
      await browser.close();
      if (fs.existsSync(opacityPath)) processedLogoPath = opacityPath;
    } catch (err) {
      console.warn('Logo opacity processing failed:', err.message);
    }
  }

  const logoSizePx = brandConfig?.logoSize || 150;
  const header = createBrandedHeader(recruiter, processedLogoPath, logoSizePx);
